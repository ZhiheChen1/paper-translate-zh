...
**手动裁图必守两条**（否则极易把图切坏，到验收才发现）：
1. **宁可裁大不可裁小**：裁切框要把图连同坐标轴、刻度、轴标签、图例、图内标题**整体框进去**，
   四边各留一点余白；只需排除英文图注那一行（中文图注由你另写）。一个常被切掉的部位是竖排的
   y 轴标签和顶部最高刻度。
2. **裁完立刻用 Read 工具看一眼裁出的 PNG**，确认上下左右没切掉任何内容；不对就调框重裁。
   不要等到最后验收才发现。
...
`OK -> ZN.pdf` 只说明**编译通过**，不代表**译文合格**。编译能过但图被裁掉一半、公式溢出版心、
表格串列、整段漏译——这些肉眼可见的毛病脚本查不出来。**必须把产出的 PDF 渲染成图，逐页用 Read
工具亲眼看过**，才能交付。这一步不是可选项。

```bash
# 把 ZN.pdf 每页渲染出来，然后用 Read 工具逐页查看
pdftoppm -png -r 130 "<workdir>/ZN.pdf" "<workdir>/_check"
```
（无本地 LaTeX、走 Overleaf 的情况：让用户编译下载后回传 PDF 再自检；若无法回传，则在交付说明里
明确告知"未经渲染自检"，并把下方清单转交用户逐项核对。）

逐页对照下面的**验收清单**，做成待办逐条勾掉：

- [ ] **图完整无裁切** —— 每张图都要看：坐标轴、刻度、图例、轴标签、图内标题有没有被切掉？
  （这是最常见的坑：裁图框小了。本仓库示例 `Schneider_Maas_zh` 的图 5 初版就把 y 轴标签和顶部
  刻度切掉了。）发现裁切 → **把裁图框放大重裁**，对照 `pages/page_NN.png` 整页图确认四边都留足，
  重新编译再看。
- [ ] **公式完整、不溢出** —— 长公式没有跑出版心右边界（overfull）；上下标、希腊字母、求和/积分
  上下限与原文一致。
- [ ] **表格完整** —— 不串列、不出界、表头与数据对齐；中文三线表线条正常。
- [ ] **中文正常显示** —— 没有方框/豆腐块/缺字（字体回退问题）。
- [ ] **引用与交叉引用解析** —— 正文无 `[?]`、`??`；`\cite`/`\cref` 都跳得到目标；参考文献列表完整。
- [ ] **内容齐全** —— 章节、图表编号连续；**没有整段漏译**，也没有残留未翻译的大段英文
  （路径 A 尤其要查：是否漏改了某些段落）。
- [ ] **无 LaTeX 报错残留** —— 日志里没有 `Undefined`、`Missing`、`No file` 等未解决项。

任一项不过关 → 修 `ZN.tex`/重裁图 → 重新编译 → **再自检**，直到全部通过。自检用的 `_check*.png`
属中间产物，交付前清掉。
...
- 有 PDF：报告 `ZN.pdf` 路径与页数，并说明已逐页渲染自检通过。
- 无本地 LaTeX：交付 `ZN.tex`（及打包好的 zip）+ 一句话的 Overleaf 编译步骤，并附上验收清单请用户过目。

简述完成度（走的哪条路径 / 全文翻译 / 含 N 图 M 表），并**诚实说明**任何未完美处理之处
（如某矢量图以页面裁图代替、某复杂表保留为图、e-print 无源码而改走 PDF）。不要谎报已完成；
自检发现但暂时修不了的问题，明确写出来而不是隐瞒。
